    ...
    while(1){
        ...
        switch(field){
            case(1):
                ...
                break;
            case(2):
                select_char_lbl(sql, "BRAND", BRAND);
                break;
            case(3):
                select_int_lbl(sql, "CAR_NUM", CAR_NUM);
                break;
            case(4):
                select_int_lbl(sql, "DESTINATION", DESTINATION);
                break;
            case(5):
                select_int_lbl(sql, "TONNAGE", TONNAGE);
                break;
            case(6):
                select_int_lbl(sql, "UNIT_PRICE", UNIT_PRICE);
                break;
            case(7):
                select_int_lbl(sql, "CARGO_COST", CARGO_COST);
                break;
            case(8):
                select_int_lbl(sql, "CATEGORY_ID", CATEGORY_ID);
                break;
            default:
...
void insert_query(char *sql) {
    ...
    enter_str(sql, "ID", ID);
    strcat(sql, ", ");
    enter_str(sql, "BRAND", BRAND);
    strcat(sql, ", ");
    enter_str(sql, "CAR_NUM", CAR_NUM);
    strcat(sql, ", ");
    enter_str(sql, "DESTINATION", DESTINATION);
    strcat(sql, ", ");
    enter_str(sql, "TONNAGE", TONNAGE);
    strcat(sql, ", ");
    enter_str(sql, "UNIT_PRICE", UNIT_PRICE);
    strcat(sql, ", ");
    enter_str(sql, "CARGO_COST", CARGO_COST);
    strcat(sql, ", ?)");
    enter_str(sql, "CATEGORY_ID", CATEGORY_ID);
    strcat(sql, ", ?)");
}
...
void select_photo(char *sql, sqlite3 *db, int mode){
    ...
    strcpy(ph_sql, "SELECT photo FROM trip WHERE ");
